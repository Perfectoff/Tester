...
const __internalLightWeightCharts= (function () {
    'use strict';
    ...
    ***************************************************************************** */
    ...
    var PriceScale = /** @class */ (function () {
        ...
        PriceScale.prototype._internal_firstValue = function () {
            ...
            for (var _i = 0, _a = this._private__dataSources; _i < _a.length; _i++) {
                ...
                //if (result !== null)
                	//if (!firstValue._internal_timePoint['valueOf']) console.log("#! ",firstValue._internal_timePoint);
					//else if (!result._internal_timePoint['valueOf']) console.log("#!! ",result._internal_timePoint);
				try {
					//if (result === null || firstValue._internal_timePoint < result._internal_timePoint) {
					if (result === null || firstValue._internal_timePoint._internal_timestamp < result._internal_timePoint._internal_timestamp) {
						result = firstValue;
					}
				}catch(e) { console.log("! ",firstValue._internal_timePoint);  console.log("!! ",result._internal_timePoint);  throw e; }
            }
    ...
    function defaultTickMarkFormatter(timePoint, tickMarkType, locale) {
        ...
        let bday= timePoint._internal_businessDay;
        var date = bday===undefined
            ? new Date(timePoint._internal_timestamp * 1000)
            : new Date(Date.UTC(bday.year, bday.month-1, bday.day, bday.hour, bday.minute, bday.second)
            );
    ...
    var FormattedLabelsCache = /** @class */ (function () {
        ...
        FormattedLabelsCache.prototype._internal_format = function (value) {
            let bday= value._internal_businessDay;
            var cacheKey = bday === undefined
                ? new Date(value._internal_timestamp * 1000).getTime()
                : new Date(Date.UTC(bday.year, bday.month - 1, bday.day, bday.hour, bday.minute, bday.second)).getTime();
    ...
    var TimeScale = /** @class */ (function () {
        function TimeScale(model, options, localizationOptions) {
            ...
            this._private__minBarSpacing = 0.5;  // Минимальное расстояние между барами (мой параметр)
            this._private__model = model;
        ...
        TimeScale.prototype._private__correctBarSpacing = function () {
            let minBarSpacing= this._private__minBarSpacing; //0.5
            if (this._private__barSpacing < minBarSpacing) { //0.5 /* MinBarSpacing */) {
                this._private__barSpacing = minBarSpacing /* MinBarSpacing */;
                this._private__visibleRangeInvalidated = true;
            }
        ...
        SeriesMarkersPaneView.prototype._internal_autoScaleMargins = function () {
            if (this._private__autoScaleMarginsInvalidated) {
                if (this._private__series._internal_indexedMarkers().length > 0) {
                    ...
                    // Непонятно зачем здесь задаются такие поля.  Пока отключили эту хуйню!
                    var marginsAboveAndBelow = calculateShapeHeight(barSpacing) * 1.5 + shapeMargin$1 * 2;
                    //console.log("!!! ",barSpacing, shapeMargin$1, marginsAboveAndBelow);
                    if (0)
    ...
    var Pane = /** @class */ function () {
        ...
        Pane.prototype._internal_recalculate = function () {
            ...
            //console.log({...this._private__rightPriceScale});
            this._private__dataSources.forEach(function (ds) {
                if (_this._internal_isOverlay(ds)) {
    ...
    function businessDayConverter(time) {
        ...
        var date = new Date(Date.UTC(time.year, time.month-1, time.day, time.hour, time.minute, time.second, 0));
        return {
    ...
    function convertTime(time) {
        ...
        if (time instanceof Date) return { _internal_timestamp: time.getTime()/1000 };
        if (!isBusinessDay(time)) {
    ...
    var validDateRegex2 = /^\d\d\d\d-\d\d-\d\d \d\d:\d\d$/;
    var validDateRegex3 = /^\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d$/;
    function stringToBusinessDay(value) {
        ...
        if (!validDateRegex.test(value) && !validDateRegex2.test(value) && !validDateRegex3.test(value)) {
            //console.log("!!!!");
            throw new Error("Invalid date string=" + value + ", expected format=yyyy-mm-dd or yyyy-mm-dd hh:mm or yyyy-mm-dd hh:mm:ss");
        }
        var d = new Date(value);
        if (isNaN(d.getTime())) {
            throw new Error("Invalid date string=" + value);
        }
        return dateToBusinessDay(d);
    }
     ...
     * @param {Date} date
     */
    function dateToBusinessDay(date) {
        if (isNaN(date.getTime())) throw new Error("Invalid date.  valueof="+ date.valueOf());
        return {
            day: date.getUTCDate(),
	...
	// Здесь убрал модификацию существующего объекта!!!
    function convertTimeStringToBusinessDay(value) {
        if (isString(value.time)) {
			value= {...value};  value.time = stringToBusinessDay(value.time);
        }
        else if (value.time instanceof Date) {
			value = {...value};
			value.time = dateToBusinessDay(value.time);
		}
        return value;
    }
	//убрал модификацию существующего объекта!!!
	/**
	 *
	 * @param{[]} data
	 * @returns {*}
	 */
    function convertTimeStringsToBusinessDays(data) {
    	//let j=0;  for(let item of data) if (!item.time) { console.log("!!!! ",j, item); throw "GGGG"; } else j++;
		let i=0;
    	for(let item of data) { let newitem= convertTimeStringToBusinessDay(item);  if (item===newitem) i++;  else break; }
		let count= data.length;
    	if (i===count) return data;
    	let newData= [...data.slice(0, i), ...new Array(count-i)];
    	for(; i<count; i++) newData[i]= convertTimeStringToBusinessDay(data[i]);
		//data.forEach(convertTimeStringToBusinessDay)
        return newData;
    }
    ...
    var DataLayer = /** @class */ (function () {
        ...
        DataLayer.prototype._internal_setSeriesData = function (series, data) {
            ...
            var seriesRows = [];
            if (data.length !== 0) {
                data = convertTimeStringsToBusinessDays(data);
        ...
        DataLayer.prototype._internal_updateSeriesData = function (series, data) {
            data = convertTimeStringToBusinessDay(data);
...
export const LightweightCharts= __internalLightWeightCharts;///window.LightweightCharts;
